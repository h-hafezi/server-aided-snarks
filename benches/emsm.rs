...
use server_aided_SNARK::gadgets::emsm::dual::{ClientState, PublicParams};
...
fn bench_msm_protocol(c: &mut Criterion) {
    ...
    for &(n, non_zeros) in &params {
        ...
        group.bench_with_input(server_new_id, &n, |b, &_n| {
            b.iter(|| {
                PublicParams::<F, G1Projective>::new(n, n, u_matrix.clone())
            });
        });

        let server_state = PublicParams::<F, G1Projective>::new(n, n, u_matrix.clone());
        ...
        group.bench_with_input(client_new_id, &n, |b, &_n| {
            b.iter(|| {
                ClientState::<F>::new(&server_state, non_zeros)
            });
        });

        let client_state = ClientState::<F>::new(&server_state, non_zeros);
        ...
        group.bench_with_input(client_phase1_id, &n, |b, &_n| {
            b.iter(|| {
                client_state.client_phase1(&server_state, witness.as_slice())
            });
        });

        let encrypted_witness = client_state.client_phase1(&server_state, witness.as_slice());
        ...
        group.bench_with_input(server_compute_id, &n, |b, &_n| {
            b.iter(|| {
                server_state.server_compute(encrypted_witness.clone())
            });
        });

        let encrypted_msm = server_state.server_compute(encrypted_witness.clone());
        ...
        group.bench_with_input(client_phase2_id, &n, |b, &_n| {
            b.iter(|| {
                client_state.client_phase2(&preprocessed_commitments, encrypted_msm.clone())
            });
        });
        ...
        group.bench_with_input(compute_plaintext_id, &n, |b, &_n| {
            b.iter(|| {
                client_state.compute_msm_in_plaintext(&server_state, witness.as_slice())
            });
        });
    }
