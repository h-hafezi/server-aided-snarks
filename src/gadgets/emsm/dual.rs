...
impl<F> ClientState<F> where
    F: PrimeField,
{
    pub fn new<G: CurveGroup<ScalarField = F>>(pp: &PublicParams<F, G>, non_zero: usize) -> ClientState<F> {
    ...
    pub fn client_phase1<G: CurveGroup<ScalarField = F>>(&self, pp: &PublicParams<F, G>, witness: &[F]) -> Vec<F> {
    ...
    pub fn client_phase2<G: CurveGroup<ScalarField = F>>(
        &self,
    ...
    pub fn compute_msm_in_plaintext<G: CurveGroup<ScalarField = F>>(&self, pp: &PublicParams<F, G>, witness: &[F]) -> G::Affine {
...
impl<F, G> PublicParams<F, G> where
    F: PrimeField,
    G: CurveGroup<ScalarField = F>
{
    pub fn new(
...
mod tests {
    ...
    use crate::gadgets::emsm::dual::{ClientState, PublicParams};
    use crate::gadgets::matrix::dense::DenseMatrix;
    ...
    fn preprocess() {
        ...
        let pp = PublicParams::<F, G1Projective>::new(n, m, u_matrix.clone());
        let client_state = ClientState::<F>::new(&pp, non_zeros);
        ...
        let encrypted_witness = client_state.client_phase1(&pp, witness.as_slice());
        ...
        let encrypted_msm = pp.server_compute(encrypted_witness);
        ...
        let msm_in_plaintext = client_state.compute_msm_in_plaintext(&pp, witness.as_slice());
